...
class CustomBlockV4 {
  ...
  CustomBlockV4(boolean lir, int results) {
	this.lir = lir;
	this.resident = true;
...
public class CustomCacheV4 extends AbstractCache {
  ...
  private LinkedHashMap<OpBGP, CustomBlockV4> stackS;
  //Q contains only resident HIR blocks
  //Index 0 represents the front, whereas the last index represents the end
  private LinkedHashMap<OpBGP, CustomBlockV4> listQ;
  private int stackLimit;
  private int queueLimit;
  
  public CustomCacheV4(int itemLimit, int resultsLimit, int stackLimit, int queueLimit) {
	super(itemLimit, resultsLimit);
	this.stackS = new LinkedHashMap<OpBGP, CustomBlockV4>();
	this.listQ = new LinkedHashMap<OpBGP, CustomBlockV4>();
  ...
  private void cycle() {
	...
	Entry<OpBGP, CustomBlockV4> y = this.stackS.entrySet().iterator().next();
  ...
  protected boolean addToCache(OpBGP bgp, OpTable opt) {
	...
	if (this.stackS.get(bgp) != null && this.listQ.get(bgp) == null) { //Case 2.1 k belongs to the stack and doesn't belong to the list
		...
		this.stackS.put(bgp, new CustomBlockV4(false, tempResults));
		CustomBlockV4 v = this.listQ.get(bgp);
  ...
  protected void removeFromCache() {
	//We search the block with the least amount of results in Q and remove it
	Iterator<OpBGP> it = this.listQ.keySet().iterator();
	OpBGP LIRSKey = it.next();
	while(it.hasNext()) {
	  OpBGP next = it.next();
	  if (this.listQ.get(next).getResults() < this.listQ.get(LIRSKey).getResults()) LIRSKey = next;
	}
	queryToSolution.remove(LIRSKey);
	listQ.remove(LIRSKey);
	//We change the status of the block X if found in the stack S
	CustomBlockV4 s = stackS.get(LIRSKey);
	if (s != null) {
	  s.setResident(false);
	  s.setLir(false);
	}
